...
class AuthorizationError extends ApplicationError {
  /**
   * The HTTP status code returned by the API if user doesn't have required permissions to access the resource
   */
  private static readonly AUTHORIZATION_HTTP_ERROR = 403;
  ...
  public override toApiError = (): ApiErrors => {
    ...
    const detail =
      ...
      (this.attribute
        ? 'with [' + this.attribute + '] = [' + this.attributeValue + '] '
        : '') +
      'is not authorized to access [' +
      this.resource +
      ']';
