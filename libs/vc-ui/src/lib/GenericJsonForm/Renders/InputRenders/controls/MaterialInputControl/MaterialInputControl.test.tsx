...
describe('MaterialInputControl', () => {
  it('should render', () => {
    ...
    const { queryAllByText } = jsonFormsTestHarness('', <MaterialInputControl input={MuiInputText} handleChange={mockCallback} {...samplePropsInputFeilds} />);
     const title = queryAllByText(samplePropsInputFeilds.label)[0];
     expect(title).toBeInstanceOf(HTMLElement);
  });
  it('should take input', () => {
    const mockCallback = jest.fn();
    const { getByLabelText } = jsonFormsTestHarness('', <MaterialInputControl input={MuiInputText} handleChange={mockCallback} {...samplePropsInputFeilds} />);
  ...
  it('should show when required', async () => {
    const mockCallback = jest.fn();
    const { findByText, getByLabelText, debug } = jsonFormsTestHarness('', <MaterialInputControl input={MuiInputText} handleChange={mockCallback} {...samplePropsInputFeildsRequired} />, true);
