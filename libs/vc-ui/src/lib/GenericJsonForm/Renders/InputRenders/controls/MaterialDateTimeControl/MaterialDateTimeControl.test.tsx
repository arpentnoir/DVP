...
describe('MaterialDateTimeControl', () => {
  it('should render', () => {
    ...
    const { queryAllByText } = jsonFormsTestHarness('', <MaterialDateTimeControl handleChange={mockCallback} {...samplePropsInputFeilds} />);
  ...
  it('should open calander selector and update on value select', () => {
    const mockCallback = jest.fn();
    const { queryByText, getByTestId, getByRole } = jsonFormsTestHarness('', <MaterialDateTimeControl handleChange={mockCallback} {...samplePropsInputFeildsRequired} />);
