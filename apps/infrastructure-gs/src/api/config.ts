...
if (
  !(
    ...
    process.env.DVP_API_INTERNAL_PATH &&
    process.env.REVOCATION_LIST_BIT_STRING_LENGTH
  )
) {
  throw new RunError(
    `Missing one or more of the required environment variables: TARGET_DOMAIN, DVP_API_INTERNAL_PATH, DVP_API_DOMAIN, REVOCATION_LIST_BIT_STRING_LENGTH`
  );
}
...
export const config = {
  ...
  apiInternalPath: process.env.DVP_API_INTERNAL_PATH,
