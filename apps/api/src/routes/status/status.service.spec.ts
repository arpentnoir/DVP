...
export const getRevocationBitLitArray = (credentialSubject: {
  id: string;
  type: string;
  encodedList: string;
}) => {
  return decodeList(credentialSubject);
};
...
describe('status.service', () => {
  ...
  describe('createRevocationList', () => {
    ...
    it('should throw error if failed to create revocation list verifiable credential', async () => {
      ...
      await expect(statusService.createRevocationListVC(2, 1)).rejects.toThrow(
        'Failed to create revocation list'
      );
    });
  });
  ...
  describe('setStatus', () => {
    ...
    it('should unrevoke a verifiable credential by setting status to 0', async () => {
      ...
      let list = await getRevocationBitLitArray(
        // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
        RevocationListRevoked.credentialSubject
      );
